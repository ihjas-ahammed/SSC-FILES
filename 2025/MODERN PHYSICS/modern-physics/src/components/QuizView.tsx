...
const QuizView: React.FC<Props> = ({ slide, onComplete }) => {
  ...
  const checkAnswer = () => {
    ...
    if (isAnswerCorrect) {
      ...
      const audio = new Audio('https://codeskulptor-demos.commondatastorage.googleapis.com/GalaxyInvaders/explosion_01.wav');
      audio.volume = 0.2;
      audio.play().catch(() => {});
    }
  };
      ...
      <div className="flex-grow overflow-y-auto no-scrollbar pb-4 px-1">
        ...
        <div className="space-y-2">
          {slide.options?.map(option => (
            ...
            >
               {/* Use inline={true} here to prevent the MathRenderer (div) from breaking the flex layout of the button */}
