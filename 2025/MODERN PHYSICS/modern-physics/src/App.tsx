...
const App: React.FC = () => {
  ...
  const [progress, setProgress] = useState<UserProgress>(() => {
    const saved = localStorage.getItem('rad_progress_v4'); // Version bump for new structure
    return saved ? JSON.parse(saved) : {
      completedLessons: [],
      xp: 0,
      currentCourseId: 'mod-1-relativity' // Default to Module 1
    };
  });
  ...
  useEffect(() => {
    localStorage.setItem('rad_progress_v4', JSON.stringify(progress));
  }, [progress]);
