...
const LessonPath: React.FC<Props> = ({ section, completedLessons, onStartLesson, onOpenSectionSelector }) => {
  ...
  // Determine unlocked lessons based on completion and "first in unit" rule
  ...
  // Effect to handle the recently completed path animation and smooth scroll
  useEffect(() => {
    ...
    if (rc) {
      ...
      if (idx !== -1 && idx < allLessons.length - 1) {
         ...
         // Smooth scroll to the new lesson right as the path animation reaches it
      ...
      <div className="mt-8 relative w-full max-w-[400px] mx-auto overflow-hidden" style={{ height: `${containerHeight}px` }}>
        ...
        {renderElements.map((el) => {
          ...
          if (el.type === 'lesson' && el.lesson && el.x !== undefined) {
             ...
             // Check if this lesson is the target of the recent completion animation
             // Just checks linear progression, which is fine for visual continuity
