  ...
  lessons: [
    {
      ...
      slides: [
        ...
        { id: "s2", type: "theory", title: "Definition of a Vector Space", content: "A **vector space** is a nonempty set $V$ of objects, called *vectors*, on which two operations are defined: **addition** and **multiplication by scalars** (real numbers).\n\nTen axioms must hold for all $\\mathbf{u}, \\mathbf{v}, \\mathbf{w}$ in $V$ and all scalars $c, d$:\n1. $\\mathbf{u} + \\mathbf{v}$ is in $V$ (Closure under addition).\n2. $\\mathbf{u} + \\mathbf{v} = \\mathbf{v} + \\mathbf{u}$.\n3. $(\\mathbf{u} + \\mathbf{v}) + \\mathbf{w} = \\mathbf{u} + (\\mathbf{v} + \\mathbf{w})$.\n4. There is a zero vector $\\mathbf{0}$ such that $\\mathbf{u} + \\mathbf{0} = \\mathbf{u}$.\n5. For each $\\mathbf{u}$, there is a $-\\mathbf{u}$ such that $\\mathbf{u} + (-\\mathbf{u}) = \\mathbf{0}$.\n6. $c\\mathbf{u}$ is in $V$ (Closure under scalar multiplication).\n7. $c(\\mathbf{u} + \\mathbf{v}) = c\\mathbf{u} + c\\mathbf{v}$.\n8. $(c + d)\\mathbf{u} = c\\mathbf{u} + d\\mathbf{u}$.\n9. $c(d\\mathbf{u}) = (cd)\\mathbf{u}$.\n10. $1\\mathbf{u} = \\mathbf{u}$." },
