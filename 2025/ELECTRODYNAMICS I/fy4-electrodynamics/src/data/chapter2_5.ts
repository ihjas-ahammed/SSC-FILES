      ...
      lessons: [
        {
          ...
          slides: [
            ...
            { id: "s1", type: "solution", title: "Interactive Solution", content: "**Goal: Apply Laplacian to the integral.**", interactiveSteps: [ { stepText: "$\\nabla^2 V = \\frac{1}{4\\pi\\epsilon_0} \\int \\rho(\\mathbf{r}') \\nabla^2 \\left( \\frac{1}{\\cal{r}} \\right) d\\tau'$." }, { stepText: "Recall that $\\nabla^2(1/\\cal{r}) = -4\\pi\\delta^3({\\cal{r}})$ (the Dirac delta)." }, { stepText: "Substitute: $\\frac{1}{4\\pi\\epsilon_0} \\int \\rho(\\mathbf{r}') (-4\\pi\\delta^3(\\mathbf{r}-\\mathbf{r}')) d\\tau'$." }, { stepText: "The $-4\\pi$ cancels. The delta function picks out $\\rho(\\mathbf{r})$. Result is $-\\rho(\\mathbf{r})/\\epsilon_0$. Goal reached! Proof complete." } ] },
