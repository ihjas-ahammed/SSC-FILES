...
export const MODULE_4: Course = {
  ...
  chapterSummary:[
    ...
    },
    {
      ...
      content: "We transitioned to formal Network Analysis, defining active/passive and linear/non-linear networks. We introduced the **Superposition Theorem**, which states that in a linear multi-source network, the total current in any branch is the algebraic sum of the currents produced by each source acting independently.\n\nWe then learned **Thevenin's and Norton's Theorems**, which allow us to dramatically simplify any linear active network down to a single voltage source and series resistor, or a single current source and parallel resistor. Finally, the **Maximum Power Transfer Theorem** proved that to extract the most power from a source, the load resistance must exactly match the source's internal Thevenin resistance ($R_L = R_{th}$)."
    }
  ]
};