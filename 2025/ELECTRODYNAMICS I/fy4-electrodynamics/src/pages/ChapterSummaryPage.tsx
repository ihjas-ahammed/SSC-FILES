...
const ChapterSummaryPage: React.FC = () => {
  const { courseId } = useParams<{ courseId: string }>();
  const navigate = useNavigate();
  const course = COURSES.find(c => c.id === courseId);

  if (!course || !course.chapterSummary) {
    return (
      <div className="min-h-screen bg-[#0b0f19] flex items-center justify-center text-slate-400">
         <div className="text-center">
             <p className="mb-4">No summary found for this chapter.</p>
  ...
  return (
    <div className="min-h-screen flex flex-col overflow-y-auto pb-20 bg-[#0b0f19] border-x border-white/5 no-scrollbar lg:max-w-3xl lg:mx-auto lg:border-x-0">
      ...
      <div className="p-5 space-y-8 mt-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
         ...
         {course.chapterSummary.map(slide => (
