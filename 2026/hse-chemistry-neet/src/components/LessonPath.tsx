  ...
  return (
    <div className="pb-32 max-w-md mx-auto relative">
      ...
      <div className="mt-8 relative w-full max-w-[400px] mx-auto overflow-hidden" style={{ height: `${containerHeight}px` }}>
        <svg viewBox={`0 0 400 ${containerHeight}`} preserveAspectRatio="none" className="absolute inset-0 w-full h-full pointer-events-none" style={{ zIndex: 0 }}>
           <PathConnector 
             points={pathPoints} completedLessons={completedLessons} recentlyCompleted={recentlyCompleted}
             unlockedLessonIds={unlockedLessonIds} colorHex={colorHex}
           />
        </svg>
        ...
        {renderElements.map((el) => {
          if (el.type === 'header') {
            return (
              <div key={`header-${el.unit.id}`} className="absolute w-full px-8 pointer-events-none" style={{ top: `${el.y}px`, transform: 'translateY(-50%)', zIndex: 10 }}>
          ...
          if (el.type === 'lesson' && el.lesson && el.x !== undefined) {
             ...
             return (
               <LessonNode key={`lesson-${el.lesson.id}`} lesson={el.lesson} x={el.x} y={el.y}
                 status={isCompleted ? 'completed' : isUnlocked ? 'current' : 'locked'}
                 isNextToStart={isNewlyCompletedPathTarget} colorName={section.color}
